  ...
  const fetchBlogs = async () => {
    try {
      ...
      if (
        error.response &&
        error.response.status === 404 &&
        error.response.data.message === "No blog found!"
      ) {
        setLoadingBlogs(false);
      }
    }
      ...
      <div className="p-5">
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 md:flex-none md:w-1/3 bg-white rounded-lg shadow-md p-6">
            ...
            <h2 className="text-base font-semibold text-green-700 mt-1 italic">
              Your Blogs:
            </h2>
            <div className="mt-0 max-h-80 overflow-y-auto">
              {loadingBlogs ? (
                ...
                blogs.map((blog, index) => (
                  <div
                    key={index}
                    className="mb-4 p-2 bg-green-50 rounded-lg mr-1"
                  >
                    <p className="text-sm font-semibold italic text-green-900 flex justify-between items-center">
                      {blog.title}
                    ...
                    <p className="text-gray-500 text-xs italic flex justify-between items-center">
                ...
                <p className="text-gray-600 text-center my-10 text-sm">
                  You currently do not have any blogs!
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 bg-white rounded-lg shadow-md p-6">
            ...
            <div className="mb-4">
              <h2 className="text-base font-semibold text-green-700 mt-6 italic">
                Your Appointments:
              </h2>
              ...
              <div className="mt-2 max-h-80   overflow-y-auto">
                {loadingSessions ? (
                  ...
                  upcomingSessions.map((session, index) => (
                    <div
                      key={index}
                      className="mb-4 px-2 py-1 rounded-lg bg-green-50 mr-1"
                    >
                      <p className="text-lg font-semibold text-green-900 italic">
                        {session.topic}
                      </p>
                  ...
                  <p className="text-gray-600 text-center my-10 text-sm ">
                    You currently do not have any sessions information!
                  </p>
                )}
              </div>
              <p
                className="text-xs font-sans italic underline text-green-600 cursor-pointer mt-2"
                onClick={() => navigate("/sessions")}
              >
                Browse sessions as viewer!
              </p>
            </div>
          </div>
        </div>
