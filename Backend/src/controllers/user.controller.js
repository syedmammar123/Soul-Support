...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (
        [fName, lName, email, password, gender].some(
            (field) => field?.trim() === ""
        )
    ) {
        res.status(400);
...
const registerPro = asyncHandler(async (req, res) => {
    ...
    if (!pro) {
        try {
            ...
            throw new ApiError(
                500,
                "Failed to register a Professional and delete its User instance from the database"
            );
        }
    }
    const createdPro = await Professional.findById(pro?._id)?.select(
        "-cv -profilePic"
    );
...
const authUser = asyncHandler(async (req, res) => {
    ...
    generateToken(res, user._id);
    return res.status(201).json(new ApiResponse(200, user, "User loggedIn"));
});
...
const getUser = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, "You have user details here!", user));
});
